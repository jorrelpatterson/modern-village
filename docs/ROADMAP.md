...
- [x] **Email drip + optimization build — LIVE IN PRODUCTION (2026-04-17)** — 18 of 20 tasks done; parent drips + optimization layer deployed. Worker version `7743ef11`. Tag `drips-deployed`. Live: screener Day 0/3/7/10, welcome Day 1/3/7, re-engagement 7/14/21, weekly digest Friday, reply webhook, conversion attribution, Thompson sampling bandit, send-time learning, auto-promote winners with 90% Bayesian gate. Admin has Edit Sequence button, Cohort Dashboard, Optimization Log, Lead Queue Manager. Remaining: Task 19 (E2E smoke — gated on Ariana editing BCBA Day 0 copy + activating the campaign), Task 20 (this doc pass, in progress). Cold sequences (BCBA/District/RC 9-step) are seeded as drafts. See [docs/superpowers/plans/2026-04-16-email-drips-STATUS.md](superpowers/plans/2026-04-16-email-drips-STATUS.md).
- [x] **Marketing AutoResearch Framework — CODE COMPLETE (2026-04-17)** — generalized experimentation system built on Phase-3 email-optimizer pattern. All 18 plan tasks done on branch `feat/email-drips-optimization`, tag `autoresearch-live`. 4 new Supabase tables (`experiment_slots`, `experiment_variants`, `experiment_assignments`, `experiment_events`), 4 worker endpoints (`/experiment/variant`, `/experiment/event`, `/experiment/link-session`, `/experiment/approve`), nightly optimizer cron with Thompson bandit + Claude challenger + auto-deploy/approval, frontend helper (`lib/experiment.js`), Meta Pixel + UTM capture, `marketing-experiments.md` config companion, admin UX (Slots Dashboard + Detail + Approval Queue + Attribution Sources). First slot (`landing_headline`) seeded with 3 variants wired into screener.html. Pending: Jorrel sets Meta Pixel ID + applies 2 migrations to Supabase + deploys worker.js. Backlog of ~10 slots to wire incrementally (landing_cta_button, paywall_heading, meta_ad_hook, onboarding_welcome, blog_cta_*). See [docs/superpowers/specs/2026-04-17-marketing-autoresearch-framework-design.md](superpowers/specs/2026-04-17-marketing-autoresearch-framework-design.md).
