...
Subagents sometimes mangle JS quote escaping in HTML `onclick=` strings. After any subagent edit to `app.html` / `admin.html`, do a syntax check (load the page in a browser, watch console). Use `\\x27` for single quotes in onclick handlers — see `feedback_quote_escaping` in Claude memory.
